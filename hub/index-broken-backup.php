...
// CHECK REAL DATA STATUS - NOT FAKE SIMULATION
// Include storage helper to check actual files
require_once __DIR__ . '/includes/storage-helper.php';
$storage = getQuietGoStorage();

// Get real user data status
$userEmail = $user['email'] ?? 'admin@quietgo.app';
$userStats = $storage->getUserStats($userEmail);
$lastSync = time() - (2 * 60); // Keep this for demo

// CHECK FOR ACTUAL UNANALYZED ITEMS
$incompleteItems = [];
$hasIncompleteData = false;

// Check for photos without analysis
if ($userStats) {
    // Create user structure to ensure paths exist
    $storage->createUserStructure($userEmail);

    // Build user path manually (since getUserPath is private)
    $safeEmail = preg_replace('/[^a-zA-Z0-9@.-]/', '_', $userEmail);
    $userPath = __DIR__ . '/QuietGoData/users/' . $safeEmail;

    // Check for meal photos that need CalcuPlate analysis (Pro+ users)
    if (in_array($subscriptionPlan, ['pro_plus'])) {
        $mealPhotosDir = $userPath . '/photos/meals';
        if (is_dir($mealPhotosDir)) {
            $unanalyzedMeals = checkUnanalyzedPhotos($mealPhotosDir, $userPath . '/analysis/ai_results');
            if ($unanalyzedMeals > 0) {
                $incompleteItems[] = [
                    'type' => 'meal',
                    'description' => "$unanalyzedMeals meal photo" . ($unanalyzedMeals > 1 ? 's' : '') . " need CalcuPlate analysis",
                    'count' => $unanalyzedMeals
                ];
            }
        }
    }

    // Check for meal photos that need manual logging (Pro users without CalcuPlate)
    if (in_array($subscriptionPlan, ['pro']) && !in_array($subscriptionPlan, ['pro_plus'])) {
        $mealPhotosDir = $userPath . '/photos/meals';
        $manualLogsDir = $userPath . '/logs/manual_meals';
        if (is_dir($mealPhotosDir)) {
            $unloggedMeals = checkPhotosWithoutManualLogs($mealPhotosDir, $manualLogsDir);
            if ($unloggedMeals > 0) {
                $incompleteItems[] = [
                    'type' => 'meal_manual',
                    'description' => "$unloggedMeals meal photo" . ($unloggedMeals > 1 ? 's' : '') . " need manual logging",
                    'count' => $unloggedMeals
                ];
            }
        }
    ...
    // Check for stool photos that need AI analysis (all Pro users)
    $stoolPhotosDir = $userPath . '/photos/stool';
    if (is_dir($stoolPhotosDir)) {
        $unanalyzedStool = checkUnanalyzedPhotos($stoolPhotosDir, $userPath . '/analysis/ai_results');
        if ($unanalyzedStool > 0) {
            $incompleteItems[] = [
                'type' => 'stool',
                'description' => "$unanalyzedStool stool photo" . ($unanalyzedStool > 1 ? 's' : '') . " need AI analysis",
                'count' => $unanalyzedStool
            ];
        }
    }

    // Set incomplete data flag based on actual findings
    $hasIncompleteData = !empty($incompleteItems);
} else {
    // If no user stats available, assume clean state
    $hasIncompleteData = false;
    $incompleteItems = [];
}

// Helper function to check for photos without corresponding analysis
function checkUnanalyzedPhotos($photosDir, $analysisDir) {
    $unanalyzedCount = 0;

    if (!is_dir($photosDir)) return 0;
    ...
    foreach ($iterator as $file) {
        if ($file->isFile() && in_array(strtolower($file->getExtension()), ['jpg', 'jpeg', 'png', 'gif'])) {
            ...
            // Check various possible analysis file locations and naming patterns
            $possibleAnalysisFiles = [
                $analysisDir . '/' . date('Y-m', $file->getMTime()) . '/' . $photoName . '_analysis.json',
                $analysisDir . '/' . $photoName . '_analysis.json',
                dirname($file->getPathname()) . '/' . $photoName . '_metadata.json'
            ];

            $hasAnalysis = false;
            foreach ($possibleAnalysisFiles as $analysisFile) {
...
// Helper function to check for meal photos without manual logs
function checkPhotosWithoutManualLogs($photosDir, $logsDir) {
    $unloggedCount = 0;
    ...
    foreach ($iterator as $file) {
        if ($file->isFile() && in_array(strtolower($file->getExtension()), ['jpg', 'jpeg', 'png', 'gif'])) {
            ...
            // Check if there's a manual log referencing this photo
            $hasManualLog = false;
            if (is_dir($logsDir)) {
                $logIterator = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($logsDir));
                foreach ($logIterator as $logFile) {
                    if ($logFile->isFile() && $logFile->getExtension() === 'json') {
                        $logData = json_decode(file_get_contents($logFile->getPathname()), true);
                        if (isset($logData['photo_filename']) && strpos($logData['photo_filename'], $photoName) !== false) {
                            $hasManualLog = true;
                            break;
                        }
                    }
